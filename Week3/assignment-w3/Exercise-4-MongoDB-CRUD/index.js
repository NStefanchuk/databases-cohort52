...
async function createEpisodeExercise(client) {
  ...
  try {
    const insertEpisode = await client
      .db('databaseWeek3')
    ...
    console.log(
      `Created season 9 episode 13 and the document got the id ${insertEpisode.insertedId}`
    )
  } catch (error) {
    console.error(
      `Something went wrong trying to insert the new episode: ${error}`
    )
  }
}
...
async function updateEpisodeExercises(client) {
  ...
  console.log(
    `Ran a command to update episode 13 in season 30 and it updated ${titleFixResult.modifiedCount} episodes`
  )
  ...
  const bushFixResult = await collection.updateMany(
    { elements: 'BUSHES' },
    { $set: { 'elements.$': 'BUSH' } }
  )
